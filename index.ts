...
const totalTasks: HTMLElement | null = document.querySelector(
  ".total-tasks span"
)!;
const completedTasks: HTMLElement | null = document.querySelector(
...
todoForm.addEventListener("submit", function (e: Event) {
  e.preventDefault();
  let input: HTMLElement = document.getElementById(
    "input"
  ) as HTMLInputElement;
...
todoList.addEventListener("keydown", function (e: KeyboardEvent) {
  if (e.keyCode === 13) {
    e.preventDefault();
  }
});
...
navigator.geolocation.getCurrentPosition((pos: GeolocationPosition) => {
  async function getWeather(): Promise<void> {
    const APIKey: string = "b0c6dd1560b603095aed754d5d1756d0";
    const APIUrl: string = `https://api.openweathermap.org/data/2.5/weather?lat=${pos.coords.latitude}&lon=${pos.coords.longitude}&appid=${APIKey}&units=metric`;
    try {
